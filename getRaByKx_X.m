function [val] = getRaByKx_X(rule,rg,a_min_range, a_max_range, b_min_range, b_max_range,binnodes)
...
for i = 1:rule
    ...
    for j = 1:rule
        ...
        numer = ((2*kx)*(((kx*ly) - (ky*lx))^2)) - (ly*((kx*kx)+(ky*ky))*((kx*ly) - (ky*lx)));
        denom = (((kx*ly) - (ky*lx))^2)*(abs((kx*ly)-(ky*lx)));

        delKByDelX = getDelKByDelX_1(binnodes,b_value);
        
        sum = sum + ((weights(i)*weights(j)*a_dt_mult*b_dt_mult*numer*delKByDelX)/denom);
    end
end
val = sum;
